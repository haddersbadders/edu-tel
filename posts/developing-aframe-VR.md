...
`` cd .\Desktop\ `` to change directory
 ...
 `` mkdir webvr_demo `` to make directory
...
Then once that's installed, to add npm: `` sudo apt install npm ``
...
For the box, we have `` position="-1 0.5 -3" `` which means the box's centre (with respect to the centre of the scene) is positioned:
...
[A-frame watcher](https://github.com/supermedium/aframe-watcher) is installed via npm (you have that installed) and will enable changes made in the inspector to be written to your HTML files. That gives us a pretty good visual editor!
 ...
 In terminal, `` sudo npm install -g aframe-watcher ``

 Then launch aframe-watcher `` aframe-watcher ``
